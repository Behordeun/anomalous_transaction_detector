...
class TestDataProcessing:
    ...
    def test_save_explained_anomalies(self):
        from analysis import NUMERIC_COLUMNS, save_explained_anomalies

        df = pd.DataFrame(
            {"user": ["user1"], "amount_value": [100.123], "anomaly_score": [0.789]}
        )
    ...
    def test_save_features_with_scores(self):
        from analysis import NUMERIC_COLUMNS, save_features_with_scores
...
class TestMainFunction:
    ...
    def test_main_rule_based(
        ...
        mock_rule_based.return_value = pd.DataFrame(
            {"anomaly_label": [1], "anomaly_score": [0.8]}
        )
    ...
    @patch("analysis.save_features_with_scores")
    @patch("analysis.create_visualisations")
    def test_main_isolation_forest(
        self,
        mock_viz,
        mock_save_features,
        mock_save_anomalies,
    ...
    @patch("analysis.save_features_with_scores")
    @patch("analysis.create_visualisations")
    def test_main_sequence_modeling(
        self,
        mock_viz,
        mock_save_features,
        mock_save_anomalies,
    ...
    @patch("analysis.save_features_with_scores")
    @patch("analysis.create_visualisations")
    def test_main_embedding_autoencoder(
        self,
        mock_viz,
        mock_save_features,
        mock_save_anomalies,
