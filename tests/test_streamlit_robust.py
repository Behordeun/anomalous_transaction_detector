...
class TestUtilityFunctions:
    def test_extract_amount_string(self):
        assert streamlit_app.extract_amount("$123.45") == 123.45
        assert streamlit_app.extract_amount("€250.75") == 250.75
        assert streamlit_app.extract_amount("100.50") == 100.50

    def test_extract_amount_numeric(self):
        assert streamlit_app.extract_amount(100.50) == 100.50
        assert streamlit_app.extract_amount(42) == 42.0

    def test_extract_amount_invalid(self):
        assert streamlit_app.extract_amount("invalid") == 0.0
        assert streamlit_app.extract_amount("") == 0.0
...
class TestEdgeCases:
    def test_extract_amount_edge_cases(self):
        assert streamlit_app.extract_amount(None) == 0.0
        assert streamlit_app.extract_amount("") == 0.0
        assert streamlit_app.extract_amount("no_numbers") == 0.0
    ...
    def test_get_risk_badge_boundary_values(self):
        ...
        level, badge = streamlit_app.get_risk_badge(5.0)
        assert level == "Medium Risk"  # 5.0 is not > 5
        
        level, badge = streamlit_app.get_risk_badge(5.1)
        assert level == "High Risk"
        
        level, badge = streamlit_app.get_risk_badge(2.0)
